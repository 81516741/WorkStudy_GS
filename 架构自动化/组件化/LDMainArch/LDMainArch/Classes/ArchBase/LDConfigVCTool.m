...
{
    UINavigationController * loginVCNavi = [[LDMediator sharedInstance] LDMediator_viewControllerForModuleLoginVC];
    if (loginVCNavi != nil) {
        //是否需要展示新特性
...
+ (void)configTabCToRootVC
{
    ...
    //模块A
    UIViewController * vcANavi = [[LDMediator sharedInstance] LDMediator_viewControllerForModuleA];
    if(vcANavi != nil) {
        [vcs addObject:vcANavi];
    }
    
    //模块B
    UINavigationController * vcBNavi = [[LDMediator sharedInstance] LDMediator_viewControllerForModuleB];
    if(vcBNavi != nil) {
...
#pragma mark - private method  LDConfigVCTool
...
- (void)configView
{
    ...
    for (int i = 0; i < images.count; i ++) {
        UIImageView * imageView = [[UIImageView alloc]initWithImage:[LDLoadMainArchSourceTool getImage:images[i]]];
        ...
        if (i == images.count - 1) {
            ...
            CGFloat width = 200;
            CGFloat height = 50;
            UIImageView * clickImageView = [[UIImageView alloc]initWithImage:[LDLoadMainArchSourceTool getImage:@"BTN_entre"]];
...
- (void)remove
{
    ...
    [LDConfigVCTool updateLocalVersion];
}
